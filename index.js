...
mobile: MethodMobile, 
browser: opcion == '1' ? ['NovaBot-MD', 'Safari', '1.0.0'] : methodCodeQR ? ['YUGI-BOT', 'Safari', '1.0.0'] : ["Ubuntu", "Chrome", "20.0.04"],
...
console.log(chalk.bgBlack(chalk.bold.redBright("⚔️ Comience con el código de país de su número de WhatsApp, ejemplo: +505 82340051"))) 
process.exit(0)
}} else {
while (true) {
addNumber = await question(chalk.bgBlack(chalk.bold.greenBright(`🤖 Ingresa el número que sera bot\nPor ejemplo: +505 82340051🤖 `)))
...
sock.sendMessage(fucker.from, { contacts: { displayName: 'YUGI-BOT👑', contacts: [{ vcard }] }}, {quoted: call, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})
await sleep(8000)
...
if (media === 'texto2')
sock.sendMessage(anu.id, { text: `\`\`\`[!] C fue ala verga : @${name.split("@")[0]} 😹\`\`\``,
contextInfo:{
forwardedNewsletterMessageInfo: { 
newsletterJid: '120363160031023229@newsletter', 
serverMessageId: '', 
